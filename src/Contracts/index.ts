...
export const getProjectDetails = async (address: string) => {
  const response = await axios.get(`${apiName}/get_project_by_owner?owner=${address}`)
  const projectId = response.data.id;
  if (projectId !== undefined || response.data.error) {
    // const res = await axios.get(`${apiName}/project_details?id=${parseInt(projectId)}`);
    const res = await axios.get(`${apiName}/project_details?id=1`);
    const data = res.data;
    return data;
  }
}
...
export const getProjectDetailsById = async (id: number) => {
  const res = await axios.get(`${apiName}/project_details?id=${id}`);
  const data = res.data;
  return data;
}
...
export const getBadgeNames = async (projectId: number) => {
  ...
  await axios.get<KingPadResponse>(`${apiName}/mainConfig?app_id=${projectId}`).then(response => {
...
export const getCreatePresaleData = async () => {
  const response = await axios.get(`${apiName}/create_presale_data?id=1`);
  const res = response.data;
  return res;
}

export const getProjectSetting = async () => {
  const response = await axios.get(`${apiName}/project_settings?id=1`);
...
export const getMainConfig = async () => {
  ...
  await axios.get<KingPadResponse>(`${apiName}/mainConfig?app_id=1`).then(response => {
